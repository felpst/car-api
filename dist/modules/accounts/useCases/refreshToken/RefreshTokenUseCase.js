...
let RefreshTokenUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(token) {
    const {
      email,
      sub
    } = (0, _jsonwebtoken.verify)(token, _auth.default.secret_refresh_token); // I have used this interface, in order to extract the sub of the methods's response.

    const user_id = sub;
    const userToken = await this.usersTokensRepository.findByUserIdAndRefreshToken(user_id, token);

    if (!userToken) {
      throw new _AppError.AppError("Refresh tokens do not exist!");
    }
    ...
    const refresh_token = (0, _jsonwebtoken.sign)({
      ...
      subject: sub,
      expiresIn: _auth.default.expires_refresh_token_days
    });
